...
Compare Analyses
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20 text-muted-foreground">
          <div className="animate-spin mr-2 h-5 w-5 border-2 border-muted-foreground border-t-primary rounded-full" />
          Loading analyses...
        </div>
      ) : error ? (
    ...
    >
      <div className="p-3 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <code className="text-sm font-semibold text-foreground">{diff.phrase}</code>
            <span
              className={cn(
                'text-[10px] px-1.5 py-0.5 rounded font-medium',
                TIER_META[diff.tier.split(' → ')[0] as keyof typeof TIER_META]?.bgColor ?? 'bg-muted',
                TIER_META[diff.tier.split(' → ')[0] as keyof typeof TIER_META]?.color ?? 'text-muted-foreground'
              )}
            >
              {diff.tier}
            </span>
      ...
      {showContext && (
        <div className="px-3 pb-3">
          <div className="bg-muted/30 rounded border border-border p-2 space-y-2">
            {diff.analysisA && (
              <div>
